...
public class CuttingMachineCategory<R extends RebornRecipe> extends AbstractMachineCategory<R> {
	...
	public List<Widget> setupDisplay(MachineRecipeDisplay<R> recipeDisplay, Rectangle bounds) {
		...
		widgets.add(Widgets.createSlot(new Point(bounds.x + 55 + 43, bounds.y + 26 + 18)).entries(getOutput(recipeDisplay, 2)).markOutput());
		widgets.add(Widgets.createSlot(new Point(bounds.x + 55 + 61, bounds.y + 26 - 18)).entries(getOutput(recipeDisplay, 3)).markOutput());
		widgets.add(Widgets.createSlot(new Point(bounds.x + 55 + 61, bounds.y + 26)).entries(getOutput(recipeDisplay, 4)).markOutput());
		widgets.add(Widgets.createSlot(new Point(bounds.x + 55 + 61, bounds.y + 26 + 18)).entries(getOutput(recipeDisplay, 5)).markOutput());
